...
test("difficulty.children.position.ext.syncAcrossViews", async (): Promise<void> => {
    ...
    const mockDeps: any = {
        ...
        estimator: {
            getScenarioRankEstimate: () => ({ continuousValue: 0 }),
            getEstimateForValue: () => ({ rankName: "Unranked", color: "#FFFFFF" }),
            calculateOverallRank: () => ({ rankName: "Unranked", color: "#FFFFFF" }),
            getScenarioContinuousValue: () => 0,
            evolveScenarioRankEstimate: () => { },
            getRankEstimateMap: () => ({}),
        },
        rankEstimator: {
            getScenarioRankEstimate: () => ({ continuousValue: 0 }),
            getEstimateForValue: () => ({ rankName: "Unranked", color: "#FFFFFF", progressToNext: 0 }),
            getRankEstimateMap: () => ({}),
        },
        appState: appState,
        folderActions: {
