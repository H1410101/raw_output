...
View `.docs/current checkpoint.md` to understand the immediate current progress. `.docs` is git ignored, and can therefore be used for internal tracking of progress. Maintain the current progress document with all tasks and architecture changes, which includes at every interaction with the user. Keep the previous progress unless your changes strictly override those.
